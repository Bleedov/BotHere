...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, LabeledPrice, PreCheckoutQuery, FSInputobżFile
...
async def menu_goblins(callback: types.CallbackQuery):
    ...
    for miner in miners:
        ...
        goblin_counts[gt] = goblin_counts.get(gt, 0) + 1
        total_profit += settings["amethyst_per_hour"]
    
    text = f"⛏ **ТВОИ ГНОМЫ**\n\n"
    if amethyst_earned > 0:
        text += f"💎 Добыто аметиста: +{amethyst_earned}\n\n"
    
    for gt, count in goblin_counts.items():
        goblin = GOBLINS[gt]
        settings = db.get_goblin_settings(gt)
        text += f"{goblin['emoji']} {goblin['name']}: {count} шт.\n"
        text +=
